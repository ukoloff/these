...
\FORALL{$\CX'\in\I_k$}
  \FORALL{cluster $V_l\in\CX\setminus\CX'$, s.t. $\CX'\cup \{V_l\}\in\I_{k+1}$}
    \FORALL{$v\in V_1$ and $u\in V_l$}
      \IF{there exists a state $S=(\CX',U,v,w)\in\L_k$, s.t. $(w,u)\in E$}
...
Each DP state  (entry of the lookup table) corresponds to a partial $v$-$u$-path and is indexed by a tuple $(\CX',V_l,v, u)$, where
\begin{itemize}
